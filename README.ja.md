...
起動時のインデックス作成では、設定された会話フォルダを優先して使用します。そのフォルダが存在しない場合は、Vault のルートにある互換性のある会話ノートをスキャンします。インデックスは各会話の本文を読み込まず、Obsidian のメタデータキャッシュから frontmatter だけを読み取ります。全文インデックスは任意で、遅延してバッチ処理され、対象ファイル数の上限を設定できます。
